...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        if (!threads.stop && !rootMoves.empty())
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool followPV) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && !is_loss(beta))
    {
        ...
        Value nullValue = -search<NonPV>(pos, ss + 1, -beta, -beta + 1, depth - R, false, false);
    ...
    if (!followPV && !allNode && depth >= 6 && !ttData.move && priorReduction <= 3)
        depth--;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
            ...
            && ttData.depth >= depth - 3 && !is_shuffling(move, ss, pos))
        {
            ...
            value = search<NonPV>(pos, ss, singularBeta - 1, singularBeta, singularDepth, cutNode, followPV);
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 4302) - (r > 5919 && newDepth > 2), !cutNode, false);
        }
        ...
        if (PvNode && (moveCount == 1 || value > alpha))
        {
            ...
            value = -search<PV>(pos, ss + 1, -beta, -alpha, newDepth, false, followPV);
        }
